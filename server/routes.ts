...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/documents/upload", isAuthenticated, upload.single('file'), async (req: any, res) => {
    try {
      ...
      try {
        const courtId = req.body.courtId || 'barnstable-probate';
  ...
  // Export court-ready PDF with compliance checks
  app.post("/api/mpc/export-pdf/:caseId", async (req, res) => {
    try {
      const { caseId } = req.params;
      ...
      const { pdfExportService } = await import('./services/pdf-export.js');
      const courtDocument = await pdfExportService.generateCourtReadyExhibitList(caseId);
      const pdfBuffer = await pdfExportService.exportToPDF(courtDocument);
      
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', `attachment; filename="Exhibit_List_${courtDocument.caseNumber}.pdf"`);
      res.send(pdfBuffer);
